...
static const char * GGML_OP_NAME[GGML_OP_COUNT] = {
    ...
    "FLASH_ATTN_EXT",
    "SPARSE_FLASH_ATTN_EXT",
    "SPARSE_FLASH_ATTN_WITH_META_EXT"
    "FLASH_ATTN_BACK",
    "SSM_CONV",
    "SSM_SCAN",
    "WIN_PART",
...
static const char * GGML_OP_SYMBOL[GGML_OP_COUNT] = {
    ...
    "flash_attn_ext(x)",
    "sparse_flash_attn_ext(x)",
    "sparse_flash_attn_with_mata_ext(x)",
    "flash_attn_back(x)",
    "ssm_conv(x)",
    "ssm_scan(x)",
    "win_part(x)",
...
// ggml_sparse_flash_attn_ext

GGML_API struct ggml_tensor * ggml_sparse_flash_attn_ext(
            struct ggml_context * ctx,
            struct ggml_tensor  * q,
            struct ggml_tensor  * k,
            struct ggml_tensor  * v,
    ...
    ggml_set_op_params(result, params, sizeof(params));
    // printf("debug 4\n");
    result->op     = GGML_OP_SPARSE_FLASH_ATTN_EXT;
    result->src[0] = q;
    result->src[1] = k;
    result->src[2] = v;
    result->src[3] = index;
...
// ggml_sparse_flash_attn_with_meta
GGML_API struct ggml_tensor * ggml_sparse_flash_attn_with_meta_ext(
            struct ggml_context * ctx,
            struct ggml_tensor  * q,
            struct ggml_tensor  * k,
            struct ggml_tensor  * v,
    ...
    ggml_set_op_params(result, params, sizeof(params));
    // printf("debug 4\n");
    result->op     = GGML_OP_SPARSE_FLASH_ATTN_WITH_META_EXT;
    result->src[0] = q;
    result->src[1] = k;
    result->src[2] = v;
    result->src[3] = index;
