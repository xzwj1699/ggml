...
static void ggml_vec_mad_f16(const int n, ggml_fp16_t * restrict y, const ggml_fp16_t * restrict x, const float v) {
#if defined(GGML_SIMD)
    const int np = (n & ~(GGML_F16_STEP - 1));
...
void ggml_print_tensor_fa(const struct ggml_tensor * tensor) {
...
static void ggml_compute_forward_flash_attn_ext_f16(
        ...
        struct ggml_tensor * dst) {
    // ggml_print_tensor_fa(q);
    // ggml_print_tensor_fa(k);
    ...
    // printf("scale %f, max_bias: %f, logit_softcap: %f\n", scale, max_bias, logit_softcap);
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        const int iq2 = (ir - iq3*neq2*neq1)/neq1; // head
        const int iq1 = (ir - iq3*neq2*neq1 - iq2*neq1); // query
        // printf("thread %d of %d, loop over n_batch and n_head: iq3: %d, iq2: %d, iq1:%d\n",ith, nth, iq3, iq2, iq1);
        ...
        // printf("thread %d of %d, loop over n_batch and n_head: ik3: %d, ik2: %d, iv3:%d, iv2:%d\n",ith, nth, ik3, ik2, iv3, iv2);
...
static void ggml_compute_forward_sparse_flash_attn_ext_f16(
        ...
        const struct ggml_tensor * k,
        const struct ggml_tensor * v,
        const struct ggml_tensor * index,
        const struct ggml_tensor * mask,
        struct ggml_tensor * dst) {
    // ggml_print_tensor_fa(q);
    // ggml_print_tensor_fa(k);
    // ggml_print_tensor_fa(v);
    // ggml_print_tensor_fa(index);
    // q:     [head_dim, 1/seq, num_head, batch]
    // k:     [head_dim, seq, num_head, batch]
    // v:     [head_dim, seq, num_head, batch]
    // index: [1, index, num_head, batch]
    // dst:   [head_dim, num_head, 1/seq, batch]
    ...
    GGML_TENSOR_LOCALS(int64_t, ne,  dst, ne)
    GGML_TENSOR_LOCALS(size_t,  nb,  dst, nb)
    GGML_TENSOR_LOCALS(int64_t, nei, index, ne)
    GGML_TENSOR_LOCALS(size_t,  nbi, index, nb)
    ...
    const int64_t ri2 = neq2/nei2;
    const int64_t ri3 = neq3/nei3;
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        float S = 0.0f;      // sum
        float M = -INFINITY; // maximum KQ value
        // printf("sparse attn execution debug 1.1\n");
        float       * VKQ32 = (float       *) params->wdata + ith*(3*D + CACHE_LINE_SIZE_F32); // FP32 VKQ accumulator
        float       * V32   =                 (VKQ32 + 1*D); // (temporary) FP32 V buffer
        ggml_fp16_t * VKQ16 = (ggml_fp16_t *) (VKQ32 + 1*D); // (temporary) FP16 VKQ accumulator
        ggml_fp16_t * Q_q   = (ggml_fp16_t *) (VKQ32 + 2*D); // (temporary) buffer for Q converted to quantized/FP16
        // printf("sparse attn execution debug 1.2\n");
        if (v->type == GGML_TYPE_F16) {
            memset(VKQ16, 0, D*sizeof(ggml_fp16_t));
        } else {
            memset(VKQ32, 0, D*sizeof(float));
        }
        // printf("sparse attn execution debug 2\n");
        const ggml_fp16_t * mp = mask ? (ggml_fp16_t *)((char *) mask->data + iq1*mask->nb[1]) : NULL;
        ...
        // index indices
        const int ii3 = iq3 / ri3;
        const int ii2 = iq2 / ri2;

        const float * pq = (const float *) ((char *) q->data + (iq1*nbq1 + iq2*nbq2 + iq3*nbq3));
        q_to_vec_dot(pq, Q_q, D);
        // printf("sparse attn execution debug 3\n");
        ...
        for (int64_t ic = 0; ic < nei1; ++ic) {
            ...
            if (ic == 0){
                for (int k = 0; k < stage; k++){
                    ...
                    char * next_k_data = (char *) k_data_base_ptr + (selected_ics[k] *nbk1);
            ...
            if (ic == 0){
                for (int k = 0; k < stage; k++){
                    char * next_v_data = (char *) v_data_base_ptr + (selected_ics[k] *nbv1);
                    const int prefetch_level = 1;
...
static void ggml_compute_forward_sparse_flash_attn_ext(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * q,
        const struct ggml_tensor * k,
        const struct ggml_tensor * v,
    ...
    switch (dst->op_params[3]) {
        ...
        case GGML_PREC_F32:
            {
                // uses F32 accumulators
                ggml_compute_forward_sparse_flash_attn_ext_f16(params, q, k, v, index, mask, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
// ggml_compute_forward_sparse_flash_attn_with_meta_ext

static void ggml_compute_forward_sparse_flash_attn_with_meta_ext_f16(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * q,
        const struct ggml_tensor * k,
        const struct ggml_tensor * v,
    ...
    const int64_t ri2 = neq2/nei2; // head
    const int64_t ri3 = neq3/nei3; // batch
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        // printf("index indices, iq3: %d, iq2: %d, ri3: %d, ri2: %d, ii3: %d, ii2: %d\n", iq3, iq2, ri3, ri2, ii3, ii2);
        ...
        // dst indices
        const int i1 = iq1; // q
        const int i2 = iq2; // num_head
        const int i3 = iq3; // batch
        // if (ith == 0) {
        //     printf("log exp debug, i1:%d, i2:%d, i3:%d\n", i1, i2, i3);
        //     printf("dst data format, nb0: %ld, nb1: %ld, nb2: %ld, nb3: %d\n", nb0, nb1, nb2, nb3);
        //     printf("dst data format, ne0: %ld, ne1: %ld, ne2: %ld, ne3: %d\n", ne0, ne1, ne2, ne3);
        //     printf("copy %f to dst: %d\n", L, (i2 + i1*ne1 + i3*ne1*ne2)*nb0 + nb3);
        // }
        ...
        // permute(0, 2, 1, 3)
        memcpy((char *) dst->data + (i3*ne2*ne1 + i2 + i1*ne1)*nb1, VKQ32, nb1);
        memcpy((char *) dst->data + (i2 + i1*ne1 + i3*ne1*ne2)*nb0 + nb3*ne3, &L, nb0);
    }
}

static void ggml_compute_forward_sparse_flash_attn_with_meta_ext(
        const struct ggml_compute_params * params,
        const struct ggml_tensor * q,
        const struct ggml_tensor * k,
        const struct ggml_tensor * v,
    ...
    switch (dst->op_params[3]) {
        ...
        case GGML_PREC_F32:
            {
                // uses F32 accumulators
                ggml_compute_forward_sparse_flash_attn_with_meta_ext_f16(params, q, k, v, index, mask, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_SPARSE_FLASH_ATTN_EXT:
            {
                ggml_compute_forward_sparse_flash_attn_ext(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor->src[3], tensor->src[4], tensor);
            } break;
        case GGML_OP_SPARSE_FLASH_ATTN_WITH_META_EXT:
            {
                ggml_compute_forward_sparse_flash_attn_with_meta_ext(params, tensor->src[0], tensor->src[1], tensor->src[2], tensor->src[3], tensor->src[4], tensor);
            } break;
